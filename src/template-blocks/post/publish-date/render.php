...
/**
 * Render function for the Publish Date Template Block.
 *
...
$bpafb_raw_date = $bpafb_post_id ? get_post_field('post_date', $bpafb_post_id) : '';
...
$bpafb_wrapper_attributes = get_block_wrapper_attributes([
	'class' => 'bpafb-tb-publish-date',
]);

printf('<span %s>', $bpafb_wrapper_attributes); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

if ($bpafb_icon) {
	echo Bpafb_Template_Block_Render::icon_html($bpafb_icon); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
